...
- **GPU-rendered window transparency does not work** (eframe glow and wgpu both composite as opaque black), and mutter puts an opaque black backdrop behind *fullscreen* windows regardless of buffer alpha. What DOES work: a **maximized** raw-Wayland surface with a software-drawn ARGB8888 `wl_shm` buffer — that is how the region-highlight outline is drawn (`src/highlight.rs`, verified by `examples/shm_overlay_probe.rs`). Its origin is the work-area origin, read from Xwayland's `_NET_WORKAREA`; global coords must be shifted by it. Recording controls still live in the main window.
