...
/// Fullscreen overlay for dragging a region selection rectangle.
///
/// Rendered as a *deferred* viewport: the fullscreen overlay fully occludes
/// the main window, and on Wayland an occluded window stops receiving frame
/// callbacks — an immediate viewport (rendered from inside the parent's
/// `update()`) would freeze along with it. A deferred viewport repaints on
/// its own, so the state lives behind an `Arc<Mutex>` that the deferred
/// callback holds a handle to.
pub struct RegionSelector {
    shared: Arc<Mutex<SelectorState>>,
}
...
struct SelectorState {
    ...
    /// Frame limiter — deferred viewports render outside App::update, so
    /// they need their own repaint-rate cap (vsync is off).
...
impl RegionSelector {
    pub fn new() -> Self {
        Self {
            shared: Arc::new(Mutex::new(SelectorState::default())),
        }
    }
    ...
    pub fn set_initial_region(&mut self, region: Option<(i32, i32, u32, u32)>) {
        self.shared.lock().unwrap().initial_region = region;
    }

    pub fn set_screenshot(&mut self, image: egui::ColorImage) {
        let mut s = self.shared.lock().unwrap();
        s.screenshot = Some(image);
        s.texture = None;
        s.drag_start = None;
        s.current_pos = None;
        s.selected_region = None;
        s.viewport_open = true;
        s.pending_rect = None;
        s.confirmed = false;
        s.confirmed_at = None;
        s.confirmed_region = None;
    ...
    pub fn take_selected_region(&mut self) -> Option<(i32, i32, u32, u32)> {
        self.shared.lock().unwrap().selected_region.take()
    }

    pub fn is_open(&self) -> bool {
        self.shared.lock().unwrap().viewport_open
    }
...
impl SelectorState {
    fn close(&mut self, ctx: &egui::Context) {
        ...
        // The occluded main window may be paused (no Wayland frame
        // callbacks), so it can't retire this viewport — destroy it from
        // here and wake the parent so it picks up the result.
    ...
    fn ui(&mut self, ctx: &egui::Context) {
        ...
        // Cap at ~144 fps, same rationale as App::update.
        ...
        let (tex_id, tex_size) = {
            let texture = self.texture.as_ref().expect("texture ensured above");
            (texture.id(), texture.size_vec2())
        };
        ...
        egui::CentralPanel::default().frame(panel_frame).show(ctx, |ui| {
            ...
            // Sense only drags: a click-and-drag sense makes egui wait for
            // ~6px of movement before drag_started(), so the selection
            // would start offset from where the button was pressed.
            let (response, painter) =
                ui.allocate_painter(available, egui::Sense::drag());
            ...
            if !confirmed {
                ...
                if response.drag_started() {
                    // press_origin is the exact press position; the pointer
                    // may already have moved by the time this frame runs.
                    let press_pos = ui
                ...
                if self.resize_handle.is_some() {
                    ...
                    if response.dragged() {
                        if let Some(pos) = response.interact_pointer_pos() {
                            let handle = self.resize_handle.unwrap();
                            if let Some(pr) = self.pending_rect {
                                self.pending_rect =
            ...
            let is_new_drag = self.drag_start.is_some();
            let sel = if is_new_drag {
                match (self.drag_start, self.current_pos) {
            ...
            let has_pending = !is_new_drag && sel.is_some() && self.resize_handle.is_none();
            ...
            if let Some(sel_rect) = sel {
                ...
                if confirmed {
                    ...
                    } else if let Some(h) = self.resize_handle {
                        ui.ctx().set_cursor_icon(cursor_for_handle(h));
                    }
